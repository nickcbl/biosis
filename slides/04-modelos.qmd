...
Modelos são sempre  aproximações  do real, gerando  _comportamento ou respostas_  também  aproximadas .
...
M  odelo de metabolismo em rede
...
A distinção entre  Parâmetro  e  Constante  é especialmente importante.
...
Equações Diferenciais Ordinárias (  ED  Os):  Concentrações de metabólitos variando continuamente no tempo (ex.: modelo de cinética enzimática de Michaelis-Menten).

Modelos Baseados em   Equações Diferenciais Parciais (EDP  s  )  :  Variação espacial contínua de gradientes de morfogenes em desenvolvimento embrionário.
...
É um modelo de rede que gera redes com alto agrupamento local (  _modularidade_  ) e baixo caminho médio (  _efeito "mundo pequeno"_  ).
...
_Watts-Strogatz (WS)_  _:_   Como é construído?
...
_Watts-Strogatz (WS)_  _:_   Aplicação no Espalhamento de Doenças Infecciosas
...
Os modelos  _Erdös-Rényi (ER)_  e  _Watts-Strogatz (WS)_  não conseguiam explicar uma propriedade fundamental de redes reais (como a internet, redes sociais e redes biológicas):  a existência de hubs (nós superconectados) . O modelo BA foi criado para explicar justamente como essas redes livres de escala surgem.
...
Ligação Preferencial (Preferential Attachment):     Quando um novo nó é adicionado, ele tende a se conectar preferencialmente a nós que já são bem conectados (que têm um grau alto).   E  feito "o rico ficam mais ricos" (rich-get-richer).
